<script lang="ts">
	...
	import { computeAdaptiveScale } from './templates/helpers';
	...
	async function downloadPdf() {
		...
		try {
			...
			// Same adaptive one-page fit as the preview: shrink the layout (zoom, which
			// html2canvas respects) so moderate CVs export on a single A4 page.
			const { scale: fitScale } = computeAdaptiveScale(el.scrollHeight);
